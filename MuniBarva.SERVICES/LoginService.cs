...
namespace MuniBarva.SERVICES
{
    public class LoginService : ILoginService
    {
        ...
        public async Task<ApiResponse<string>> Send(SendEmailRecoverPasswordDTO recoverPassword)
        {
        ...
        public async Task<ApiResponse<EmployeesDTO>> SignIn(string _email, string _password)
        {
            ...
            {
                var rols = await _loginDAO.GetRols(oEmployee.Id);

                var oEmployeesDTO = oEmployee.Adapt<EmployeesDTO>();

                if (rols is not null && rols.Count > 0)
                {
                    var rolsDTO = rols.Adapt<List<RolsDTO>>();

                    oEmployeesDTO.Rols = rolsDTO;

                    oEmployeesDTO.Jwt = await GetJwT(oEmployeesDTO, rolsDTO);
                }
        ...
        public async Task<ApiResponse<string>> RecoverPassword(RecoverPasswordDTO recoverPassword)
        {
            recoverPassword.Password = await _encrypt.Sha256(recoverPassword.Password);

            var employees = recoverPassword.Adapt<Employees>();

            await _loginDAO.RecoverPassword(employees);

            return new ApiResponse<string> { Message = "La contraseña ha sido actualizada con éxito." };
        ...
        public async Task<ApiResponse<VerifyTokenDTO>> VerifyToken(string _token)
        {
            ...
            return new ApiResponse<VerifyTokenDTO> { Data = result, Message = "" };
        }

        private async Task<string> GetJwT(EmployeesDTO _employeesDTO, List<RolsDTO> _rols)
        {
            return await Task.Run(() =>
            {
                ...
                foreach (var rol in _rols)
                {
                    claims.Add(new Claim(ClaimTypes.Role, rol.Description));
                }
